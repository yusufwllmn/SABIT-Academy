...
public:
    ...
    bool Login(string nomorRekening, string pin, int &index, Validasi &validasi){
        if (!validasi.ValidasiDigit(nomorRekening, 10)){
            cout << "Nomor rekening harus 10 digit angka" << endl;
            return false;
        }
        
        if (!validasi.ValidasiDigit(pin, 6)){
    ...
    void Daftar (Validasi &validasi){
        ...
        while(true){
            ...
            bool cekNomorRekening = validasi.ValidasiDigit(nomorRekening, 10);
            bool cekPin = validasi.ValidasiDigit(pin, 6);
...
class Transaksi{
public:
    void CekSaldo(int index, vector<Akun> &akun) {
        cout << "\n============= Saldo =============" << endl;
        cout << "Saldo anda: Rp" << akun[index].saldo << endl;
        cout << "=================================" << endl;
    }

    void SetorTunai(int index, vector<Akun> &akun, Validasi &validasi){
        int jumlah;
        ...
        if(jumlah >= 50000 && validasi.ValidasiKelipatan(jumlah)){
    ...
    void TarikTunai(int index, vector<Akun> &akun, Validasi &validasi) {
        int jumlah;
        int pilihan;
        ...
        if (jumlah >= 50000 && validasi.ValidasiKelipatan(jumlah) && jumlah <= akun[index].saldo){
            akun[index].saldo -= jumlah;
            cout << "=================================" << endl;
    ...
    void Transfer(int index, vector<Akun> &akun, Validasi &validasi){
        ...
        while(true){
            ...
            if(!validasi.ValidasiDigit(rekeningTujuan, 10)) {
                cout << "=================================" << endl;
...
class Menu {
public:
    int TampilMenu(int index, vector<Akun> &akun){
        int pilihan;
    ...
    void Run() {
        ...
        do{
            ...
            if(pilihan == 1){
                ...
                cout << "Masukkan PIN: ";
                cin >> pin;
                if(akunManager.Login(nomorRekening, pin, index, validasi)){
                    cout << "=================================" << endl;
                    cout << "Login berhasil" << endl;
                    cout << "=================================" << endl;
                    do{
                        pilihan = menu.TampilMenu(index, akunManager.akun);
                        if(pilihan == 1){
                            transaksi.CekSaldo(index, akunManager.akun);    
                        }else if (pilihan == 2){
                            transaksi.SetorTunai(index, akunManager.akun, validasi);  
                        }else if (pilihan == 3){
                            transaksi.TarikTunai(index, akunManager.akun, validasi);
                        }else if (pilihan == 4){
                            transaksi.Transfer(index, akunManager.akun, validasi);     
                        }else if (pilihan == 5){
                            cout << "=================================" << endl;
